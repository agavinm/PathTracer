...
public:
    ...
    public:
        ...
        Node(const vector<Real> &_p, const T &_data) : p(_p), d(_data), axis(-1) {
            if (_p.size() != N) throw ("Wrong dimension number in kd-tree");
        }

        const vector<Real> &point() const {
            return p;
        }
...
public:
    ...
    void store(const vector<Real> &point, const T &data) {
        nodes.push_back(Node(point, data));
    }

private:
    static void median_split(vector<Node> &p, int start, int end, int median, int axis);

    static void balanceSegment(vector<Node> &pbal, vector<Node> &porg, int index, int start, int end, const vector<Real> &bbmin, const vector<Real> &bbmax);

public:
    void balance();

    static Real distance(const vector<Real> &p1, const vector<Real> &p2) {
        Real sol2 = 0.0;
...
private:
    int closest(const vector<Real> &p, int index, int best) const;

    void find(const vector<Real> &p, int index, Real radius, list<const Node *> &nodes) const;

    void find(const vector<Real> &p, int index, int nb_elements, float &dist_worst, vector<const Node *> &nodes, vector<pair<int, float> > &dist) const;
    ...
    void update_heap_nodes(const Node &node, const float distance, int nb_elements, vector<const Node *> &nodes, vector<pair<int, float> > &dist) const;

public:
    //========================================================================================================
    // Fixed Radius
    int find(const vector<Real> &p, Real radius, list<const Node *> *nodes) const {
        if (nodes) {
    ...
    void find(const vector<Real> &p, int nb_elements, vector<const Node *> &nodes, Real &max_distance) const {
        nodes.clear();
        max_distance = numeric_limits<Real>::infinity();
    ...
    const Node &find(const vector<Real> &p) const {
...
//Private Find(radius)
template<class T, unsigned int N>
void KDTree<T, N>::find(const vector<Real> &p, int index, Real radius, list<const Node *> &nodes) const {
...
template<class T, unsigned int N>
void KDTree<T, N>::update_heap_nodes(const Node &node, const float distance, int nb_elements, vector<const Node *> &nodes, vector<pair<int, float> > &dist) const {
...
template<class T, unsigned int N>
void KDTree<T, N>::find(const vector<Real> &p, int index, int nb_elements, float &dist_worst, vector<const Node *> &nodes, vector<pair<int, float> > &dist) const {
    Real aux;
...
// Closest
template<class T, unsigned int N>
int KDTree<T, N>::closest(const vector<Real> &p, int index, int best) const {
    int sol = best;
    Real distbest = distance(p, balanced[best].point());
    Real aux;
    //We check if our node is better
    if ((aux = distance(balanced[index].point(), p)) < distbest) {
        sol = index;
        distbest = aux;
    }
    //Now we check that this is not a leaf node
...
//Balance Tree
template<class T, unsigned int N>
void KDTree<T, N>::median_split(vector<Node> &p, int start, int end, int median, int axis) {
    int left = start;
    int right = end;
...
template<class T, unsigned int N>
void KDTree<T, N>::balanceSegment(vector<Node> &pbal, vector<Node> &porg, int index, int start, int end, const vector<Real> &bbmin, const vector<Real> &bbmax) {
    int median = 1;
...
void KDTree<T, N>::balance() {
    {
        vector<Node> aux(nodes.size() + 1);
        balanced.resize(nodes.size() + 1);
        int i;
        vector<Real> bbmax = nodes.front().point();
        vector<Real> bbmin = nodes.front().point();
